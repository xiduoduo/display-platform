...
const proDir = require('../config/pro-dir-config')
// 根据项目输出不同目录 - config/index.js 同时修改
let dir = proDir('../dist');
// let dir = '../dist';
...
let getStaticRoutes = (routes = [], root_path = '') => {
  ...
  routes.forEach(elem => {
    ...
    if (elem.children) {
      let rss = getStaticRoutes(elem.children,
...
const PROJECT = process.env.PROJECT ? '-' + process.env.PROJECT : '';
// const env = process.env.NODE_ENV === 'testing'
//   ? require('../config/test.env')
//   : require('../config/prod.env')
const env = process.env.NODE_ENV === 'testing'
  ? require('../config/test.env')
  : require('../config/prod' + PROJECT + '.env');
  ...
  plugins: [
    ...
    ...dynamicRoutes.map(elem => {
      return new HtmlWebpackPlugin({
        filename: path.resolve(__dirname, dir + elem),
        template: 'index.html',
        // inject: 'head',
        minify: {
          removeComments: true,
