<template>
  <section class="global-part logistics flex">
    ...
    <div class="right"
      ...
      <LogisticTrend class="right-item" :dataRef="trendData" :animation="stopAnimate" :themeOption="themeOption"></LogisticTrend>
    </div>
  </section>
</template>
...
import LogisticsService from '@/services/energy/logistics';
...
export default {
  ...
  methods: {
    ...
    async bindSummaryData () {
      let _self = this;
      LogisticsService.getEnergySummary().subscribe(result => {
    ...
    bindMapData () { // 地图数据
      let _self = this;
      LogisticsService.getEnergyMapData().subscribe(result => {
        if (result.data.type === 'success') {
    ...
    bindRiskWarnData () { // 累计风险预警
      let _self = this;
      LogisticsService.getEnergyRiskWarnData().subscribe(result => {
        if (result.data.type === 'success') {
    ...
    bindLatestArrivedList () { // 绑定到达运单
      let _self = this;
      LogisticsService.getEnergyLastArrived().subscribe(result => {
    ...
    bindLatestRunningList () { // 已发车运单
      let _self = this;
      LogisticsService.getEnergyLastStart().subscribe(result => {
    ...
    bindGrade () { // 累计运输评价
      let _self = this;
      LogisticsService.getEnergyScore().subscribe(result => {
        if (result.data.type === 'success') {
          // 缓存-并-赋值
    ...
    bindTrend () { // 累计运单走势
      let _self = this;
      LogisticsService.getEnergyTrend().subscribe(result => {
        if (result.data.type === 'success') {
          // 缓存-并-赋值
