<template>
  <section class="global-part flex part-three">
    <div class="left">
      ...
      <div class="btm-file-block display-flex animated bounceInUp" :class="{bounceOutDown: lever}">
        <NumberBlock :theme="6" class="right-num">
          <div class="run-num">
            <RunNum :runData="power"/>
          </div>
        </NumberBlock>
        <NumberBlock :theme="6" class="right-num">
          <div class="run-num">
            <RunNum :runData="reduce"/>
          </div>
        </NumberBlock>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getRunSummary () { // 总时长、公里数
      let _self = this;
      let obs = await GetIndexServices.getEnergyRunSummary();
      this.closeSub = obs.subscribe(res => {
        if (res.data.type === 'success') {
          ...
          _self.power.num = data.total_power;
          _self.reduce.num = data.total_reduce;
          _self.mileage = JSON.parse(JSON.stringify(_self.mileage));
          _self.hours = JSON.parse(JSON.stringify(_self.hours));
          _self.power = JSON.parse(JSON.stringify(_self.power));
          _self.reduce = JSON.parse(JSON.stringify(_self.reduce));
        } else {
    ...
    async getRunAnalysis () { // 商用车行驶-数据
      let _self = this;
      let obs = await GetIndexServices.getEnergyRunAnalysis();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          let mileageCom = [];
    ...
    async getAvgVelocity () { // 商用车行驶-车速
      let _self = this;
      let obs = await GetIndexServices.getEnergyAvgVelocity();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          let avgSpeedCom = [];
    ...
    async getLocList () { // 地图热区
      let _self = this;
      let obs = await GetIndexServices.getEnergyLocList();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          // 缓存
    ...
    async getDealer () { // 经销商
      ...
      if (_self.dealer.length) {
        ...
        let obs = await GetIndexServices.getEnergyDealer();
        obs.subscribe(res => {
          if (res.data.type === 'success') {
            let data = res.data.data;
