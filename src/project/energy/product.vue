...
<script>
import GetProductServices from '@/services/energy/getProduct';
...
export default {
  ...
  data () {
    return {
      ...
      themeOption: {
        ...
        effectItemColor: this.GREEN.echarts.map.effectItemColor2,
        scatterShow: false
      },
      dayCarNum: {
        title: '今日生产:',
  ...
  methods: {
    ...
    async getProdTop20 () {
      let _self = this;
      let obs = await GetProductServices.getEnergyProdTop20();
      obs.subscribe(res => {
    ...
    async getProdCBrand () {
      ...
      if (_self.currentPageBrand.name === BRAND_MAP.ALL) {
        // 车品牌 === 全部
        let obs = await GetProductServices.getEnergyProdCBrand();
        obs.subscribe(res => {
          if (res.data.type === 'success') {
            let data = res.data.data;
        ...
        // 车品牌 != 全部
        let obs = await GetProductServices.getEnergyProdCType();
        obs.subscribe(res => {
          if (res.data.type === 'success') {
            let data = res.data.data;
