<template>
  <section class="global-part flex part-two">
    ...
    <div class="mid animated zoomIn" :class="{zoomOut: lever}">
      <Map :mapData="mapData" :type="Boolean(true)" :animation="stopAnimate"/>
    </div>
    <div class="right animated bounceInRight" :class="{bounceOutRight: lever}">
      <ul>
        <li class="right-block-btm">
          <NumberBlock :theme="2" v-if="currentPageBrand.name !== BRAND_MAP.OUHUI" class="right-block">
            <h3 class="global-block-title text-left">当月不规范驾驶行为分析</h3>
            <Radar :setData="radarData" :animation="stopAnimate"/>
          </NumberBlock>
          <NumberBlock :theme="2" v-else>
            <h3 class="global-block-title text-left">行为分析</h3>
...
import { DRIVE_CODES, BRAND_MAP, ALARM_CODE_MAP, FAULT_WARN_CODES } from '@/services/common';
  ...
  methods: {
    getData () {
      if (this.getMenuList.length) {
        ...
        this.getMonthBehavior();
        this.getDayAlarm();
        this.getDistribute();
        if (this.currentPageBrand.name === BRAND_MAP.OUHUI) {
          this.getOuManRunAnalysis();
        }
      }
    },
    getOuManRunAnalysis () { // 采集
      let _self = this;
      // let obs = await GetIndexServices.getOuManRunAnalysis();
      this.closeSub = GetIndexServices.getOuManRunAnalysis().subscribe(res => {
        if (res.data.type === 'success') {
          ...
          let xDataCom = [];
          // 行驶-数据
          data.CommVehicle.slice(-10).forEach(ele => {
          ...
          // 缓存
          _self.cachePage.getOuManRunAnalysis = _self.lineBarCommVehicle;
          // 赋值
          _self.lineBarCommVehicle = JSON.parse(JSON.stringify(_self.lineBarCommVehicle));
        } else {
          if (_self.cachePage.getOuManRunAnalysis) {
            let data = _self.cachePage.getOuManRunAnalysis;
    ...
    getDataAmount () { // 采集
      let _self = this;
      // let obs = await GetIndexServices.getDataAmount();
      this.closeSub = GetIndexServices.getDataAmount().subscribe(res => {
        if (!res.data.error) {
    ...
    getMonthFaultWarn () { // 本月故障报警
      let _self = this;
      // let obs = await GetIndexServices.getMonthFaultWarn();
      GetIndexServices.getMonthFaultWarn().subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          let xData = [];
          let useData = [];
    ...
    getMonthBehavior () { // 本月不规范驾驶行为
      let _self = this;
      // let obs = await GetIndexServices.getMonthBehavior();
      GetIndexServices.getMonthBehavior().subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          let indicator = {};
          data.forEach(ele => {
            // console.log(ele);
            let reg = new RegExp(ele.name);
            if (reg.test(DRIVE_CODES.join(','))) {
              indicator[ele.name] = ele.count;
            }
          });
          _self.radarData.data[0].indicator = indicator;
          // 缓存
          _self.cachePage.getMonthBehavior = _self.radarData;
          // 赋值
          _self.radarData = JSON.parse(JSON.stringify(_self.radarData));
        } else {
          if (_self.cachePage.getMonthBehavior) {
            let data = _self.cachePage.getMonthBehavior;
            _self.radarData = JSON.parse(JSON.stringify(data));
          }
        }
      }, e => console.log(e));
    },
    getDayAlarm () { // 今日报警分析
      let _self = this;
      // let obs = await GetIndexServices.getDayAlarm();
      GetIndexServices.getDayAlarm().subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          let newData = [];
          // 按产品需求筛选数据
