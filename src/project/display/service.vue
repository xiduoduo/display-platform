    ...
    <div class="left animated bounceInLeft" :class="{bounceOutLeft: lever}">
      ...
      >
        <div class="run-num-line">
          <InfoLine :theme="2" :infoLineData="infoLineData"
                    :isActived="isActived"/>
        </div>
      </NumberBlock>
    </div>
...
import GetService from '@/services/display/getService';
...
export default {
  ...
  methods: {
    getData () {
      if (this.getMenuList && this.getMenuList.length) {
    ...
    getInfoLineData () {
      ...
      let falseLineData = FAULTS_WARN.filter(ele => {
        return this.currentPageBrand.name === BRAND_MAP.ALL
          ? Boolean(true)
          : this.currentPageBrand.name === ele.carBrandName;
    ...
    async getInStationCar () { // 地图数据
      let _self = this;
      let obs = await GetService.getInStationCar();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let data = res.data.data;
          if (!data.length) { // 如果为空时
            _self.runMapData = {
              title: '今日进站车辆：',
              line: true,
              num: 0,
    ...
    async getFaultRanking () { // 常见故障
      let _self = this;
      let obs = await GetService.getFaultRanking();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          ...
          // 排序-截取部分数据-json去重
          let toHeavy = {};
          resData.sort((a, b) => b.count - a.count).forEach(ele => {
            if (!toHeavy[ele.name]) {
              toHeavy[ele.name] = ele;
            }
          });
          // 得到去重结果-根据WARN_CODE_SERVICE筛选数据
          let getHeavyData = [];
          Object.values(toHeavy).forEach(ele => {
            if (WARN_CODE_SERVICE[ele.code] !== undefined) {
              getHeavyData.push({
          ...
          // 整理数据-截取0-20条数据-赋值
          _self.wordClouds = getHeavyData.slice(0, getLen);
        }
      }, e => console.log(`service: ${e}`));
    },
    async getAlarmRanking () { // 报警统计Top5
      let _self = this;
      let obs = await GetService.getAlarmRanking();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let resData = res.data.data;
          let resLen = resData.length;
    ...
    async getRepairByPrvRank () { // 服务真实性
      let _self = this;
      let obs = await GetService.getRepairByPrvRank();
      obs.subscribe(res => {
        if (res.data.type === 'success') {
          let resData = res.data.data;
          let resLen = resData.length;
          ...
          // 排序-截取部分数据
          let sliceData = resData.sort(
            (a, b) => Number(b.value) - Number(a.value)).slice(0, 5);
          // 整理数据-赋值
